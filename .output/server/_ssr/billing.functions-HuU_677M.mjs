import { n as createServerFn } from "./server-B47OKpt8.mjs";
import { t as requireSupabaseAuth } from "./auth-middleware-CbYYwkHC.mjs";
...
import { t as createServerRpc } from "./createServerRpc-CzwK31cr.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/billing.functions-HuU_677M.js
...
var verifyCheckout = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({ reference: stringType().min(6).max(120) }).parse(d)).handler(verifyCheckout_createServerFn_handler, async ({ data, context }) => {
	...
	if (existing?.status !== "success") {
		...
		const { data: sp } = await supabaseAdmin.from("subscription_payments").select("id").eq("reference", data.reference).maybeSingle();
		if (sp?.id) try {
