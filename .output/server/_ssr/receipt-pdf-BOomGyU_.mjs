...
//#region node_modules/.nitro/vite/services/ssr/assets/receipt-pdf-BOomGyU_.js
var import_lib = /* @__PURE__ */ __toESM(require_lib());
function receiptUrl(publicId) {
	return `${typeof window === "undefined" ? "" : window.location.origin}/receipt/${publicId}`;
}
async function buildReceiptPdf(receipt) {
	...
	doc.text(new Date(receipt.issued_at).toLocaleDateString("en-GB"), w - 40, 68, { align: "right" });
	...
	row("Property / Unit", `${snap.property ?? "—"}  ·  Unit ${snap.unit ?? "—"}${snap.room ? ` · Room ${snap.room}` : ""}`);
	...
	const qr = await import_lib.toDataURL(url, {
		margin: 1,
		width: 320
	});
...
async function downloadReceiptPdf(receipt) {
	(await buildReceiptPdf(receipt)).save(`${receipt.receipt_number}.pdf`);
}
async function qrDataUrl(value) {
	return import_lib.toDataURL(value, {
		margin: 1,
		width: 320
	});
}
//#endregion
export { qrDataUrl as n, receiptUrl as r, downloadReceiptPdf as t };
